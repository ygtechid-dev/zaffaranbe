...
class ProductController extends Controller
{
    ...
    {
        // Sesuaikan dengan struktur folder Hostinger kamu
        $publicPath = base_path('public');
        return $publicPath . '/uploads/' . $folder;
    }

    private function getImageUrl(string $folder, string $filename): string
    {
    ...
    public function uploadImage(Request $request, $id)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($product->getRawOriginal('image')) {
                $oldPath = base_path('public') . $product->getRawOriginal('image');
            ...
            DB::table('products')->where('id', $id)->update(['image' => $imageUrl]);

            return response()->json([
                'message' => 'Image uploaded successfully',
                'image_url' => $imageUrl,
                'product' => $product->fresh()->load(['branches', 'variants.stocks', 'stocks'])
            ]);
        }
